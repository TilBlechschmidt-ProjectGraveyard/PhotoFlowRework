...
class RepresentationViewController: UIViewController {
    ...
    public var representationData: RepresentationData? {
        didSet {
            if let representationData = self.representationData {
                self.rendererView?.removeFromSuperview()
                
                let rendererView = ImageRendererView(data: representationData.data, imageBoundsView: imageBoundsView)
                rendererView.map {
                    view.addSubview($0)
                    view.bringSubviewToFront(imageBoundsView)
...
//        imageBoundsView.snp.makeConstraints { make in
//            make.left.equalToSuperview()
//            make.right.equalToSuperview()
//            make.top.equalTo(view.safeAreaLayoutGuide.snp.top)
//            make.bottom.equalToSuperview()
